...
| **Monitoreo de los servicios externos que consume el POS** (17-sep-2026) | El log del POS decia que algo fallo; faltaba saber DONDE se corta la cadena. `bucle_servicios_pos` en el agente lee el `.exe.Config` real y chequea los dos PostgreSQL (con `SELECT 1`, no el puerto: Postgres acepta la conexion mucho antes de poder atender consultas), Odoo y el SOAP de recargas — este ultimo encontrado por coincidencia de nombre, porque el completo incluye el namespace de .NET. Se mide desde la ESTACION: una base central sana con la ruta rota desde una farmacia es, para esa farmacia, una base caida, y desde el servidor se veria perfecta. El agente reporta datos crudos y no decide umbrales — eso vive en `ReglaAlerta`, o cambiarlo exigiria redistribuir el ejecutable a ~1.800 equipos. `EstadoServicioPos` (estado actual) + `MuestraServicioPos` (serie de latencia), separados siguiendo el precedente `EstadoEnlaceFarmacia`/`MuestraRedFarmacia`. El campo `critico` decide si aplica la regla critica o la de advertencia, sin inventar una severidad nueva. La contrasena de Postgres nunca sale del proceso del agente y el modelo no tiene donde guardarla | ✅ Hecho |
...
| **Apertura autoprovisionada de farmacias ("cero-touch")** (11-sep-2026) | Iniciativa del usuario: estandarizar la apertura de una farmacia como un despliegue automatizado en vez de una visita técnica que configura cada estación a mano (las 8 del piloto se hicieron así y cada una destapó un bug distinto). App nueva `apps/aperturas`, que **no ejecuta trabajo nuevo: orquesta el que ya existe** — cada `PasoPlantilla` delega en `apps.scripts`, `apps.software`, `apps.despliegues` o `apps.activos`. Modelos: `PlantillaApertura` (versionada, por unidad de negocio/grupo/formato) con sus `PerfilEstacionPlantilla` (qué estaciones se esperan, por sufijo del código, y con qué flags quedan) y `PasoPlantilla` ordenados; `Apertura` (instancia por farmacia+fecha, con regla de cuatro ojos vía permiso `aprobar_apertura`, y constraint parcial `una_apertura_vigente_por_farmacia`); `PasoApertura` + `EventoApertura` inmutable (mismo contrato que `ResultadoDespliegue`/`EventoDespliegue`). **El gancho cero-touch es `TokenApertura`**: secreto de un solo uso por estación esperada, con vencimiento, del que **solo se guarda el hash** — `manejar_enrolamiento` (`apps/mqtt_worker/services.py`) gana una rama que, con token válido, crea la estación YA APROBADA con la config de su perfil y lanza sus pasos. Responde directo a §10-Z: el paquete "un clic" anterior llevaba los secretos compartidos de toda la flota adentro y quedó publicado sin auth; un token filtrado ahora compromete una estación de una apertura, no la flota. Validaciones del token: vigente + farmacia del código == farmacia de la apertura + sufijo == perfil emitido (sin la segunda, un token filtrado enrola en cualquier farmacia; sin la tercera, un token de caja se lleva los flags del servidor). Un token inválido **no** rechaza el enrolamiento: cae al camino de siempre (pendiente de aprobación manual), para que un `config.txt` mal copiado no deje al técnico sin poder enrolar. Aprobación automática auditada (`apertura.estacion_enrolada`, con el prefijo del token). Verificaciones que leen lo que la estación ya reporta, sin pedirle nada nuevo al agente (BitLocker, software presente en el inventario de R7, Windows Update escaneado, versión de POS objetivo, coherencia del nodo). Al completarse escribe `Farmacia.fecha_apertura`. Reusos con cambio mínimo en módulos existentes: `publicar_despliegue_a_estacion` nuevo en `apps/despliegues/services.py` (suma una estación recién enrolada a un despliegue YA aprobado, por su tópico individual — publicar al tópico de la farmacia reinstalaría el POS en las cajas que ya lo aplicaron) y kwarg `estaciones` en `crear_activos_desde_estaciones`. Comando `sincronizar_aperturas`: los pasos no se enteran solos de su resultado (el worker MQTT actualiza `ResultadoEjecucionScript`/`ResultadoInstalacion` sin saber que pertenecen a una apertura), así que la apertura va y lee, en vez de acoplar el worker a este módulo. **Fuera de alcance a propósito, y documentado como manual en vez de fingido como automático**: el enlace de datos lo provisiona el proveedor (el Mikrotik no lo toca el agente), el alta en AD necesita una credencial de dominio en la estación (mismo problema del secreto compartido, sin resolver) y el 2FA es de persona, no de máquina | 🟡 Backend hecho (899 tests OK **en SQLite** — el Postgres local no estaba levantado; falta la corrida contra PostgreSQL, que es la que cuenta). Lado agente escrito (`--token-apertura` / `TokenApertura` en `config.txt`, propagado por `Instalar.bat` e `instalar-servicio.ps1`; el token nunca se escribe en el log local ni en `identidad.json`) pero **deliberadamente diferido para el final**: el `.exe` NO se reconstruyó ni se redistribuyó, y `VERSION_AGENTE_PRUEBA` sigue en `agente-prueba-0.17` — hay que bumpearla a 0.18 antes de redistribuir, o la flota reportaría la misma versión para dos binarios distintos. El servidor se puede desplegar solo: la rama de enrolamiento solo se ejecuta si el payload trae `token_apertura`, y ningún agente en campo lo manda todavía, así que el módulo queda **inerte en producción** hasta que se reconstruya el agente y se emita el primer token. Panel `/aperturas/` hecho (lista, alta, detalle con polling HTMX cada 5 s igual que el progreso de un despliegue; aprobar, emitir/revocar tokens, cerrar pasos manuales con evidencia, reintentar un paso en error, cancelar —que revoca los tokens sin usar, o la cancelación sería cosmética—). El token en claro se muestra en la respuesta del POST que lo emite y en ningún otro lado, con una prueba que lo fija (`AperturaPanelTests.test_el_token_en_claro_se_muestra_una_sola_vez`). La plantilla se edita en el admin a propósito: se define una vez por formato y casi no cambia. Dos comandos, ambos simulan por defecto: `seed_plantilla_apertura` (plantilla usable de formato mostrador — no inventa scripts: los pasos de script/software solo se crean si se le pasa un `Script`/`AplicacionCatalogo` existente; BitLocker y Windows Update quedan NO obligatorios porque muchas cajas son Windows Home o no tienen salida a internet, y no pueden trabar la apertura de un local) y `generar_paquete_apertura` (arma el `config.txt` de cada estación esperada con su token adentro; **se niega a escribir dentro de `MEDIA_ROOT`**, que es donde quedó publicado el paquete del §10-Z, y no reemite un token ya entregado porque el valor en claro no se puede releer). Límite conocido y documentado de ese paquete: sigue llevando `MqttPassword` y `COMANDO_HMAC_SECRET`, compartidos por toda la flota — el token es por estación, esos dos no; cerrarlo pide HMAC por estación, que es un cambio aparte. **Pendiente: el rebuild del agente (bump a 0.18) y su redistribución.** `seed_permisos` hay que correrlo a mano tras desplegar (el entrypoint solo hace `migrate`+`collectstatic`), o el grupo Administrador no recibe `aprobar_apertura`/`emitir_token_apertura`. Bloqueado para piloto por la falta de ruta de red a las farmacias — un equipo enchufado en un local nuevo todavía no alcanza al broker |
| **Notificaciones por Telegram y diagnóstico automático con IA** (17-sep-2026) | Pedido del usuario: avisar al teléfono y, en las alertas críticas, ayudar a identificar la causa antes de escalar. Telegram entra por el motor de alertas que ya existía (`CanalNotificacion` tipo `telegram`, misma resolución global/por unidad que el webhook de Teams), no como un bot aparte: la escalada llega gratis porque pasa por el mismo `notificar_alerta`. Prefijo 🔴/🟡 por severidad, para decidir en la lista de chats del teléfono cuál abrir primero. El resumen de enlaces usa un chat propio (`ENLACES_TELEGRAM_CHAT_ID`) y no el canal global, por el mismo motivo que `ENLACES_NOTIFICAR_A` no reusa los destinatarios de las alertas: 196 caídas en 24 h sobre 700 sitios es otro orden de magnitud. El diagnóstico con Claude corre solo en alertas CRÍTICAS, una vez por incidente (lo garantiza que `abrir_o_mantener_alerta` no reabre una alerta activa — control de costo, no solo de ruido) y de forma asíncrona, para que una API lenta no retrase la apertura ni el aviso. El resultado se guarda aparte de todo dato operativo y se manda rotulado como automático y sin verificar: es una hipótesis, y el prompt le prohíbe al modelo inventar una causa cuando el contexto no alcanza. Sin `TELEGRAM_BOT_TOKEN` o sin `ANTHROPIC_API_KEY`, cada parte queda desactivada sin romper el resto.  El mismo dia se sumaron las consultas de vuelta (`/enlaces`, `/estado`, `/alertas`, `/farmacia`), que estaban fuera del alcance inicial: worker de long polling, solo lectura y con lista blanca de chats obligatoria, porque un bot de Telegram es publico y lo que devuelven esas consultas es el mapa de la red. Ampliado el 18-sep-2026 con `/criticas`, `/mantenimiento` y `/toperrores` (este ultimo excluye los errores de categoria NEGOCIO, igual que el motor de alertas: una validacion rutinaria del POS con miles de repeticiones tapaba cualquier bug real) y un resumen diario a las 8:00 que sale por CanalNotificacion global — no por la lista de chats autorizados, que es control de acceso entrante y no un destino. **Arreglado tambien el `migrate` duplicado de `desplegar.sh`**: el entrypoint de `web` ya migra al arrancar y el script lo repetia "porque migrate es idempotente". Lo es en secuencia, no en concurrencia — `up -d` vuelve apenas arrancan los contenedores y los dos aplicaban la misma migracion a la vez, sin lock, con el segundo reventando en `column already exists`. Paso dos veces el 17-sep-2026 (0025 y 0026) y las dos hubo que verificar a mano si la base habia quedado sana. Ahora el paso solo MUESTRA las lineas de migracion del arranque; que no queden pendientes lo sigue decidiendo la verificacion final, que es de solo lectura.|
| **Auditoría de bugs y lógica de negocio** (18-sep-2026) | Revisión sistemática de las 16 apps contra un checklist de aislamiento multi-tenant, CASCADE sobre historiales, agregaciones con JOIN, motor de alertas, idempotencia MQTT, convención `--aplicar`, RBAC y husos horarios. **Cinco hallazgos verificados** (no teóricos: cada uno medido contra el código real y, donde se pudo, contra producción). **(1)** `EventoEnlaceFarmacia.farmacia` era CASCADE: 690 de 700 farmacias no tenían ningún `PROTECT` que las frenara, así que el 99% del historial de SLA —1038 de 1046 eventos— dependía de que nadie tocara "eliminar". Igual en `EventoMantenimiento`, que además declara `delete()` como `NotImplementedError` y el CASCADE evadía esa garantía porque Django no instancia los hijos al cascadear. Ambos a `PROTECT`; la baja de una farmacia es `activa=False`, y una sin historial se sigue pudiendo borrar. **(2)** El agente guardaba la posición del log del POS *antes* de publicar: con el broker caído (se reconecta varias veces al día) paho descartaba el mensaje con QoS 0 y esos errores no se reintentaban nunca — ni llegaban al panel ni los contaba `evaluar_regla_pos_errores`, así que su alerta podía no dispararse. Agente 0.24: la posición avanza solo tras confirmar el envío; se elige duplicar antes que perder. **(3)** El bug de huso horario corregido en mantenimiento (9f9ba9a) seguía vivo en 7 lugares: `now().date()` devuelve la fecha en UTC y con `America/Guayaquil` eso está mal 5 de cada 24 horas — `ActividadCumplimiento.vencida` marcaba vencido cinco horas antes, y los programados de scripts y de inventario agendaban con un día corrido. Los tests del proyecto usaban el mismo patrón, o sea que eran flaky y habrían fallado corriendo de noche. **(4)** Dos comandos tenían `--dry-run` en vez de `--aplicar`, o sea el default invertido: había que acordarse de pedir el simulacro, y correr `crear_tecnicos_soporte` sin flags creaba nueve colaboradores y nueve usuarios con login al panel. Ahora simulan por defecto; `--dry-run` se sigue aceptando por compatibilidad, ya redundante. Hay un test que recorre todos los comandos y falla si alguno vuelve a invertir el default. **(5)** `ActividadPlanificada` no tenía `unidad_negocio`: la lista mostraba las de todos los clientes y completar aceptaba cualquier id de la URL. No había fuga real (0 filas, y los 3 usuarios con el permiso tienen acceso global), pero se armaba sola al poner el módulo en uso. Campo nuevo (nullable = actividad interna, visible para todos, mismo criterio que `Script`), scope en la lista, `verificar_acceso` al completar, y el desplegable del formulario acotado a las unidades del usuario. |

**Monitoreo de enlaces (`Cresio_enlaces`) — evaluado el 11-sep-2026, ver
`docs/evaluacion-cresio-enlaces.md`:** el sistema anterior de monitoreo de enlaces (el
mismo que menciona el docstring de `importar_circuitos_proveedor`) estaba **vivo hasta el
4-sep-2026** sondeando por ICMP las **704 sucursales** de la flota, con ciclo de ~60s y
alertas por WebSocket y correo. Es la capacidad que SAIDSOFT no tiene: monitorear **sin
agente instalado**, que hoy cubre 8 de ~1.800 estaciones.

Lo más importante de ese hallazgo no es el código —la carpeta que llegó **no trae el
paquete `app/` ni el `src/` del frontend**, así que no arranca— sino que **alcanzaba IP
privadas de las farmacias**. El bloqueante nº 1 de este proyecto es justamente que el
servidor no tiene ruta hacia esas IP. Alguna máquina de la organización sí la tiene:
averiguar cuál y por qué es la acción de mayor valor pendiente, porque destrabaría el
